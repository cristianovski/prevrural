...
type PeriodoType = 'rural' | 'urbano' | 'beneficio' | 'lacuna' | 'prova de retorno'; // ALTERAÇÃO: adicionado 'prova de retorno'
...
export function AnalysisPage({ cliente, onBack }: AnalysisPageProps) {
  ...
  const loadAllData = async () => {
    ...
    try {
      ...
      if (interviewData) {
        ...
        // ALTERAÇÃO: Não adicionamos mais documentos da ficha de entrevista na lista lateral
      }

      // ALTERAÇÃO: Não adicionamos mais documentos do campo legado 'personal_docs'

      // 3. BUSCAR DA NOVA TABELA (client_documents) - APENAS CATEGORIA "Provas"
      const newDocs = newDocsRes.data;
      if (newDocs) {
        const docsDb = newDocs
          .filter((doc: Record<string, any>) => doc.category === 'Provas') // Filtro por categoria
        ...
        docsColetados = [...docsColetados, ...docsDb];
      }

      // Ordenar tudo por data (issueDate)
      docsColetados.sort((a, b) => new Date(a.issueDate).getTime() - new Date(b.issueDate).getTime());
      setDocumentos(docsColetados);
  ...
  const calcularTotais = () => {
    ...
    periodos.forEach(p => {
      ...
      // ALTERAÇÃO: 'prova de retorno' não conta nos totais (pode ajustar conforme necessidade)
    });
    setTotalRural(rural);
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      ...
      <main className="flex-1 overflow-hidden flex flex-col md:flex-row">
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
              <div className="md:col-span-3">
                ...
                <select value={form.tipo} onChange={e => setForm({...form, tipo: e.target.value as PeriodoType})} className="w-full p-2.5 border rounded-lg bg-white outline-none focus:border-amber-500 text-sm">
                  ...
                  {/* ALTERAÇÃO: nova opção adicionada */}
          ...
          <div className="space-y-3">
            {periodos.map((p) => {
              ...
              // ALTERAÇÃO: visual para 'prova de retorno' (pode personalizar)
              else if (p.tipo === 'prova de retorno') { bgColor = "bg-purple-50"; borderColor = "border-purple-200"; icon = <Paperclip size={18} className="text-purple-600"/>; statusText = "Prova de Retorno"; }
        ...
        {/* BARRA LATERAL DIREITA - DOCUMENTOS DE PROVA */}
        <div className="w-full md:w-80 bg-slate-100 border-l border-slate-200 p-4 overflow-y-auto hidden md:block">
          ...
          {documentos.length === 0 ? (
            <div className="text-center py-10 text-slate-400 text-xs">
              Nenhum documento da categoria "Provas" encontrado.
            </div>
          ) : (
            <div className="space-y-3">
              {documentos.map((doc, i) => {
                // ALTERAÇÃO: formatar data para dd/mm/aaaa
                let dataFormatada = '';
                  ...
                  <div key={`${doc.id}-${i}`} className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm hover:border-blue-300 transition-colors group">
                    <div className="flex justify-between items-start mb-1">
                      <span className="bg-slate-100 text-slate-600 text-[10px] font-bold px-1.5 py-0.5 rounded">
                        {dataFormatada} {/* ← agora mostra dia/mês/ano */}
                      </span>
                      {doc.fileUrl && (
