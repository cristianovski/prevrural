...
// Tipagem para ajudar no desenvolvimento
interface Client {
  id: number;
  nome: string;
  cpf: string;
...
export function ClientListPage({ onSelectClient, onNewClient }: ClientListPageProps) {
  ...
  // Busca os clientes (O RLS do Supabase garante que venham apenas os seus)
  ...
  const getStatusColor = (status: string | undefined) => {
      switch(status) {
          ...
          default: return 'bg-slate-100 text-slate-500 border-slate-200'; // A Iniciar
      }
  };
          ...
          {loading ? (
             ...
             <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-200 flex flex-col items-center gap-4">
                ...
                <div>
                    ...
                    <button onClick={onNewClient} className="text-emerald-600 font-bold hover:underline text-sm">Cadastrar Cliente</button>
                </div>
             </div>
          ) : (
